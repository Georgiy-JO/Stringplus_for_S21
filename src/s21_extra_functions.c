...
//   -src - one char long
//   -trim="a" src=
//   ""/"b"/"a"/"aa"/"ab"/"ba"/"aaa"/"aba"/"aab"/"baa"/"abba"/"aabb"/"baaa"/"aaabaaa"/"aaabbbaaa"/"aaaaaa"
...
size_t head_tail_delta_finder(const char *src, const char *trim_chars,
  ...
  if (tail_point != 0) {
    ...
    {
        local_flag=0;
        if(s21_isinstr(src[head_point], trim_chars))    head_point++; else
    local_flag++; if(s21_isinstr(src[tail_point], trim_chars))    tail_point--;
    else local_flag++; } while (local_flag<2 && head_point<tail_point);
    */
    do {
